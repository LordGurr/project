...
function Header({ cartCount, currentPage, setCurrentPage, customer, onLogout }) {
    return ( <
        ...
        <
        button style = { currentPage === 'products' ? styles.navButtonActive : styles.navButton }
        onClick = {
            () => setCurrentPage('products') } >
        Products <
        /button>
        ...
        {
            customer && ( <
                button style = { currentPage === 'orders' ? styles.navButtonActive : styles.navButton }
                onClick = {
    ...
    function ProductCard({ product, onAddToCart }) {
        ...
        return ( <
            ...
            div style = { styles.productImage } > {
                product.category_name === 'Drinks' ? '🥤' : product.category_name === 'Frozen Food' ? '🍕' : product.category_name === 'Ice Cream' ? '🍦' : product.category_name === 'Snacks' ? '🍿' : '🍫'
            } <
            /div>
            ...
            <
            span style = { styles.productPrice } > { priceKr }
            kr < /span>
...

            {
                product.stock > 0 && product.stock < 10 && ( <
                    p style = { styles.lowStock } > Only { product.stock }
                    left! < /p>
                )
            }

            {
                product.average_rating && ( <
                    p style = { styles.rating } > { '⭐'.repeat(Math.round(product.average_rating)) }({ product.review_count }) <
                    /p>
                )
            } <
            /div> <
    ...
    function ProductsPage({ onAddToCart }) {
        ...
        return ( <
            ...
            option value = "" > All Categories < /option> {
                categories.map(cat => ( <
                    option key = { cat.id }
                    value = { cat.id } > { cat.name }({ cat.product_count }) <
                    /option>
                ))
            } <
            /select>
            ...
            checked = { inStockOnly }
            onChange = {
                (e) => { setInStockOnly(e.target.checked);
                    setPage(1); } }
            />
            In stock only <
            /label> <
                ...
                loading ? ( <
                    ...
                    { /* Pagination */ } {
                        totalPages > 1 && ( <
                            ...
                            ←Previous <
                            /button> <
                            span style = { styles.pageInfo } > Page { page }
                            of { totalPages } < /span> <
                            button style = { styles.pageButton }
                            onClick = {
                        ...
                        orders.map(order => ( <
                            ...
                            div style = { styles.orderItems } > {
                                order.items.map(item => ( <
                                    ...
                                    span > { item.subtotal_kr.toFixed(2) }
                                    kr < /span> <
                                    /div>
                                ))
                            } <
                            ...
                            span style = { styles.orderTotal } > { order.total_amount_kr.toFixed(2) }
                            kr < /span> <
                            /div> <
                            /div>
                        ))
    ...
    function AuthPage({ onLogin, setCurrentPage }) {
        ...
        return ( <
...

            {
                ...
                disabled = { loading } > { loading ? 'Please wait...' : (isRegister ? 'Register' : 'Login') } <
                /button> <
                /form>

                <
                p style = { styles.authSwitch } > { isRegister ? 'Already have an account? ' : "Don't have an account? " } <
                button
        ...
        function Toast({ message, onClose }) {
            ...
            return ( <
                div style = { styles.toast } > { message } <
                /div>
            );
        }
        ...
        export default function App() {
            ...
            return ( <
                ...
                <
                Header cartCount = { cart.item_count }
